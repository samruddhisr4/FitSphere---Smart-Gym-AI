...
const WorkoutPlan = () => {
  ...
  const {
    completedExercises,
    toggleExerciseCompletion: contextToggleExerciseCompletion,
    calculateOverallProgress,
    resetProgress,
  } = useExerciseCompletion();

  // Calculate progress using context
  const progress = workoutPlan?.weeklySchedule
    ? calculateOverallProgress(workoutPlan.weeklySchedule)
    : { completed: 0, total: 0, percentage: 0, remaining: 0 };
  ...
  const toggleExerciseCompletion = (dayIndex, exerciseIndex) => {
    ...
    if (day && exercise) {
      contextToggleExerciseCompletion(
        dayIndex,
        exerciseIndex,
        exercise.name,
        day.day
          ...
          const transformedPlan = {
            ...
            weeklySchedule: plan.workouts
              ? (Array.isArray(plan.workouts)
                  ...
                  const isRestDay =
                    workout.description &&
                    workout.description.toLowerCase().includes("rest");
                  ...
                  return {
                    ...
                    exercises: isRestDay
                      ...
                      : [
                          {
                            name: workout.description || "Sample Exercise",
                            details: "3 sets of 8-12 reps",
                          },
                          {
                            name: "Core Exercise",
                            details: "4 sets of 6-10 reps",
                          },
                        ],
                  };
                })
              : [
                  ...
                  },
                  {
                    ...
                    type: "active",
                    exercises: [
                      { name: "Pull-ups", details: "3 sets of 6-10 reps" },
                    ],
                  },
                  {
                    day: "Wednesday",
    ...
    try {
      ...
      if (response.success) {
        ...
        const transformedPlan = {
          ...
          weeklySchedule: plan.weeklySchedule
              ...
              plan.weeklySchedule.map((day, index) => {
                ...
                if (day.exercises && Array.isArray(day.exercises)) {
                  exercises = day.exercises.map((exercise, exIndex) => ({
                    name: exercise.name,
                    details: `Sets: ${exercise.sets || 3}, Reps: ${
                      exercise.reps || "8-12"
            ...
            ? (Array.isArray(plan.workouts)
                ...
                if (exerciseDetails && exerciseDetails.exercises) {
                  exercises = exerciseDetails.exercises.map(
                    (exercise, exIndex) => ({
                      name: exercise,
                      details: `${exerciseDetails.sets || 3} sets of ${
                        exerciseDetails.reps || "8-12"
                      } reps, ${exerciseDetails.rest || "60-90 sec"} rest`,
                    })
                  );
                } else if (!isRestDay) {
                ...
                return {
                  ...
                  focus: isRestDay
                    ? "Rest Day"
                    : workout.description || "Detailed Workout",
                  type: isRestDay ? "rest" : "active",
            ...
            : [
                // Fallback schedule if no workouts provided
                {
                  ...
                  exercises: [
                    { name: "Bench Press", details: "3 sets of 8-12 reps" },
                    { name: "Push-ups", details: "3 sets of 12-15 reps" },
                  ],
                  duration: 45,
                },
                {
                  ...
                  exercises: [
                    { name: "Pull-ups", details: "3 sets of 6-10 reps" },
                    { name: "Rows", details: "3 sets of 8-12 reps" },
                  ],
                  duration: 45,
                },
                {
                  ...
                  exercises: [
                    { name: "Squats", details: "3 sets of 8-12 reps" },
                    { name: "Lunges", details: "3 sets of 10 reps each leg" },
                  ],
                  duration: 45,
                },
                {
                  ...
                  exercises: [
                    { name: "Shoulder Press", details: "3 sets of 8-12 reps" },
                    { name: "Planks", details: "3 sets of 30-60 seconds" },
                  ],
                  duration: 45,
                },
                {
                  ...
                  exercises: [
                    { name: "Deadlifts", details: "3 sets of 6-10 reps" },
                    { name: "Burpees", details: "3 sets of 8-12 reps" },
                  ],
                  duration: 50,
                },
              ],
          ...
          upcomingSessions:
            plan.workouts && plan.workouts.length > 0
              ? (Array.isArray(plan.workouts) ? plan.workouts : [plan.workouts])
                  .slice(0, 3)
                  .map((w, index) => ({
                    day:
                      w.name ||
              ...
              <div className="space-y-4">
                {workoutPlan.weeklySchedule?.map((day, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h4 className="font-medium text-white">
                          {day.focus && day.day && day.focus.includes(day.day)
                            ? day.focus
                            : day.day && day.focus
                              ? `${day.day} - ${day.focus}`
                              : day.day || `Day ${index + 1}`}
                        ...
                        {day.muscleGroups && day.muscleGroups.length > 0 && (
                          <p className="text-xs text-gray-400 mt-1">
                            Target Muscles: {day.muscleGroups.join(", ")}
                          </p>
                        )}
                      </div>
                    ...
                    {day.type !== "rest" && day.exercises?.length > 0 ? (
                      <div className="mt-3">
                        ...
                        <ul className="text-sm text-gray-400 space-y-2 mb-4">
                          {day.exercises.map((exercise, exIndex) => {
                            ...
                            const isGenericMuscleGroup =
                              exercise.name &&
                              (exercise.name
                                .toLowerCase()
                                .includes("chest & triceps") ||
                                exercise.name
                                  .toLowerCase()
                                  .includes("back & biceps") ||
                                exercise.name.toLowerCase().includes("legs") ||
                                exercise.name
                                  .toLowerCase()
                                  .includes("shoulders & abs") ||
                                exercise.name
                                  .toLowerCase()
                                  .includes("full body") ||
                                exercise.name.toLowerCase().includes("core"));

                            let displayExerciseName = exercise.name;
                            
                            // Only modify if it's a generic placeholder
                            if (isGenericMuscleGroup) {
                              if (exercise.name.toLowerCase().includes("chest & triceps")) {
                                displayExerciseName = "Bench Press";
                              } else if (exercise.name.toLowerCase().includes("back & biceps")) {
                                displayExerciseName = "Pull-ups";
                              } else if (exercise.name.toLowerCase().includes("legs")) {
                                displayExerciseName = "Squats";
                              } else if (exercise.name.toLowerCase().includes("shoulders & abs")) {
                                displayExerciseName = "Shoulder Press";
                              } else if (exercise.name.toLowerCase().includes("full body")) {
                                displayExerciseName = "Burpees";
                              } else if (exercise.name.toLowerCase().includes("core")) {
                                displayExerciseName = "Planks";
                              } else {
                                displayExerciseName = exercise.name.replace(" & ", " and ");
                              }
                            }

                            const completionKey = `${index}-${exIndex}`;
                            const isCompleted =
                              completedExercises[completionKey] || false;
                            const videoUrl =
                              getEmbeddedVideoUrl(displayExerciseName);

                            return (
                              <li
                                key={exIndex}
                                className="flex items-start gap-2 bg-dark-700 p-3 rounded hover:bg-dark-600 transition-colors"
                              >
                                <button
                                  onClick={() =>
                                    toggleExerciseCompletion(index, exIndex)
                                  }
                      ...
                      <p className="text-sm text-gray-400">
                        Complete recovery day - focus on sleep, hydration, and
                        nutrition
                      </p>
                    )}
                  </div>
                  ...
                  <div className="text-center py-8 text-gray-400">
                    No schedule available. Generate a new plan to see your
                    workout schedule.
                  </div>
                )}
              </div>
